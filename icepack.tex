...
In C++ one could avoid modifying the diagnostic solve routine by several different means.
The function signature could be expressed using variadic templates.
This is an advanced language feature and relying on it would present a steep learning curve for novices.
The inputs could also be wrapped up in a class.
Users can then define a subclass if they need to change the input fields.
This would be a fairly idiomatic approach for the language but requires more knowledge on the part of users of object-oriented programming and its conventions than the approach we'll describe below.
...
At this rate there is no marginal benefit over using a dynamically-typed language such as Python.
