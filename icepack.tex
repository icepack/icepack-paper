...
\end{enumerate}
To accomplish these tasks, these tools are usually written in compiled programming languages such as C, C++, and Fortran for reasons of computational efficiency.
...
In this paper, we introduce a new Python software package for glacier flow modeling called \emph{iceapck}.
...
More recently, \citet{schoof2005effect} proposed the functional form
\begin{equation}
    \tau_b = -C\left(\frac{|u|}{|u| + u_c}\right)^{\frac{1}{m}}\frac{u}{|u|}
\end{equation}
where $u_c$ is some critical speed.
This sliding law acts like the Weertman law when $|u| \ll u_c$ and like the plastic law when $|u| \gg u_c$.
This latter equation has been found to agree best with laboratory experiments \citep{zoet2020slip}.
...
Of the programming languages that are commonly used for scientific computing, only Python and possibly Julia would appear meet these needs.
...
The icepack source repository is at https://github.com/icepack/icepack and the git commit hash of the version of the code used for this publication is 250490a528c4abe473825bebbc823b2de0c43000.
