...
Starting from a guess $u_k$ for the velocity, one first computed a \emph{search direction} $v_k$ that solves a linear equation:
\begin{equation}
    \ud^2J(u_k)\cdot v_k = -\ud J(u_k).
    \label{eq:newton-search-direction}
\end{equation}
The next value of the velocity is then computed by searching along $v_k$:
\begin{equation}
    u_{k + 1} = u_k + \alpha_k\cdot v_k,
\end{equation}
where $\alpha$ is chosen to minimize $J$ along the search direction.
...
Several works in the literature have weighed the relative merits of different iterative methods for solving the nonlinear diagnostic equation, but few if any have considered the problem of when to stop iterating.
...
Finally, submarine melting is turned off for at least 200 years, optionally longer.
...
Rather than specify the sliding law as such, icepack expresses the sliding law as the derivative of some action functional.
In other words, to implement the Schoof sliding law in icepack, we need to know the antiderivative of equation \eqref{eq:schoof-sliding}.
...
The power dissipation density for this sliding law is
\begin{equation}
...
To demonstrate the forward and inverse modeling capabilities of icepack, we will estimate the rheology of the Larsen C Ice Shelf in the Antarctic Peninsula from observational data and project the model forward by 10 years.
...
By estimating the material hardness or rheology of an ice shelf, we can constrain where marine ice may be forming.
