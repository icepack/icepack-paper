...
The evolution equations for ice thickness and velocity are necessary for any simulation, but other fields with their own dynamics can be a part of the problem as well.
...
Where there is ablation in ice-free regions, a correctly-implemented prognostic solver will compute a negative thickness, which is unphysical and requires correction.
The prognostic solver in icepack truncates the thickness to zero, which approximates the free boundary, where there is ablation in ice-free regions.
An implicit approach would instead treat the free boundary problem directly as a variational inequality \citep{jouvet2012steady}.
Icepack currently lacks such a scheme and this will be the subject of future development.
