...
The SIA model \citep{hutter1981effect} describes the interior of ice sheets well, where flow can be simply described as bed-parallel shear and surface and basal slopes are small. This approximation does not apply well near ice divides, where vertical velocities dominate, and near ice margins, where basal sliding can be a large fraction of the surface speed. 
...
The shallow ice model can be derived from the Blatter-Pattyn approximation by assuming the ice flow is dominated by bed-parallel shear and that surface and basal slopes are small, i.e. $\partial u/\partial x \ll \partial u/\partial z$. The result is a 2D system of equations for depth-averaged horizontal velocity. The terms in the action functional are:

\begin{align}
    \text{mass} & =\frac{1}{2}\int_\Omega u\cdot u\ud x\\\
    \text{gravity} & = \int_\Omega\frac{2A(\varrho_I g)^n}{n+2} (\nabla h^2\cdot u) h^{n+1} \nabla s^{n-1}\ud x\\
    \text{penalty} & = \frac{1}{2}\int_\Omega l^2\nabla u\cdot \nabla u\ud x 
\end{align}

where 

\begin{equation}
l = 2 \text{ max}(\text{cell diameter}, 5h)
\end{equation}
...
We have included an additional penalty term, which helps alleviate numerical artifacts, especially near the ice margins and termini where the shallow ice approximation is less applicable.

The shallow ice approximation applies well in ice-sheet interiors. For this reason, and because the equations are particularly simple to solve, this approximation has been a common choice for ice-sheet modeling \textcolor{red}{cite}. Caution must be taken, however, because areas of the ice sheet that experience high vertical velocities (e.g., ice divides) or plug flow (e.g., ice streams) are not well represented by this approximation.
...
The shallow ice and shallow stream models in icepack are completely standard. \textcolor{red}{JB: Aren't the models in icepack based on variational principles? This sentence and the next suggest that only the hybrid model is based on variational principles.}
